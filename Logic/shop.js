...
class ShopService extends EventEmitter {
	...
	deleteShop(shop_id) {
		logger.log("INFO", "[Logic/Shop]deleteShop shop_id:%d ", shop_id);

		if (this.__shopState.has(shop_id)) {
			this.__shopState.delete(shop_id)
		}
		let uid = this.getUidByShopId(shop_id)
		if (uid > 0 && this.__ownShop.has(uid)) {
			this.__ownShop.delete(uid);
		}

		if (this.__pendingShop.has(shop_id)) {
			this.__pendingShop.delete(shop_id);
		}

		uid = this.getCliamPlayer(shop_id)
		if (uid > 0 && this.__shopClaim.has(uid)) {
			this.__shopClaim.delete(uid);
		}
